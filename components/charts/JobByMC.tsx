...
import { filterCostRows, computeJobsByMC } from '@/lib/dataUtils';
...
export function JobByMC({ costRows, loading }: Props) {
  ...
  const data = useMemo(() => {
    ...
    return computeJobsByMC(filtered);
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400 mt-0.5">Number of test jobs per machine category</p>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 8, left: 8, bottom: 4 }}>
            ...
            <Bar dataKey="value" radius={[6, 6, 0, 0]} maxBarSize={60}>
              {data.map((_, idx) => (
                <Cell key={idx} fill={CHART_COLOR_LIST[(idx + 2) % CHART_COLOR_LIST.length]} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
